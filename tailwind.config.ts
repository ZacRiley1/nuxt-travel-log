import type { Config } from 'tailwindcss'
import daisyui from 'daisyui'

const config: Config = {
  content: [
    './components/**/*.{vue,js,ts}',
    './layouts/**/*.vue',
    './pages/**/*.vue',
    './plugins/**/*.{js,ts}',
    './stores/**/*.{js,ts}',
    './app.vue',
  ],
  theme: {
    extend: {},
  },
  plugins: [daisyui],
  daisyui: {
    themes: [
      {
        dark: {
          'color-scheme': 'dark',
          'base-100': 'oklch(21% 0.006 285.885)',
          'base-200': 'oklch(27% 0.006 285.885)',
          'base-300': 'oklch(35% 0.006 286.033)',
          'base-content': 'oklch(96% 0.003 264.542)',
          primary: 'oklch(22% 0.006 286.033)',
          'primary-content': 'oklch(87% 0.006 286.286)',
          secondary: 'oklch(76% 0.177 163.223)',
          'secondary-content': 'oklch(27% 0.046 192.524)',
          accent: 'oklch(50% 0.118 165.612)',
          'accent-content': 'oklch(98% 0 0)',
          neutral: 'oklch(0.2265 0.0173 184.24)',
          'neutral-content': 'oklch(98% 0 0)',
          info: 'oklch(70% 0.14 182.503)',
          'info-content': 'oklch(30% 0.056 229.695)',
          success: 'oklch(62% 0.194 149.214)',
          'success-content': 'oklch(26% 0.065 152.934)',
          warning: 'oklch(75% 0.183 55.934)',
          'warning-content': 'oklch(26% 0.079 36.259)',
          error: 'oklch(71% 0.194 13.428)',
          'error-content': 'oklch(27% 0.105 12.094)',
          'radius-selector': '0.25rem',
          'radius-field': '0.3rem',
          'radius-box': '0.25rem',
          'size-selector': '0.21875rem',
          'size-field': '0.21875rem',
          border: '1px',
          depth: '1',
          noise: '0',
        },
      },
      {
        light: {
          'color-scheme': 'light',
          'base-100': 'oklch(96% 0.001 286.375)',
          'base-200': 'oklch(92% 0.001 286.375)',
          'base-300': 'oklch(88% 0.004 286.32)',
          'base-content': 'oklch(0.1765 0.0074 184.24)',
          primary: 'oklch(87% 0.006 286.286)',
          'primary-content': 'oklch(0.2529 0.0123 184.24)',
          secondary: 'oklch(84% 0.143 164.978)',
          'secondary-content': 'oklch(26% 0.051 172.552)',
          accent: 'oklch(50% 0.118 165.612)',
          'accent-content': 'oklch(98% 0 0)',
          neutral: 'oklch(0.2265 0.0173 184.24)',
          'neutral-content': 'oklch(98% 0 0)',
          info: 'oklch(60% 0.126 221.723)',
          'info-content': 'oklch(98% 0.019 200.873)',
          success: 'oklch(59% 0.145 163.225)',
          'success-content': 'oklch(97% 0.021 166.113)',
          warning: 'oklch(68% 0.162 75.834)',
          'warning-content': 'oklch(98% 0.026 102.212)',
          error: 'oklch(40% 0.4 0.9)',
          'error-content': 'oklch(97% 0.014 343.198)',
          'radius-selector': '0.25rem',
          'radius-field': '0.5rem',
          'radius-box': '0.25rem',
          'size-selector': '0.21875rem',
          'size-field': '0.21875rem',
          border: '1px',
          depth: '1',
          noise: '0',
        },
      },
      'lemonade',
      'dim',
      'sunset',
    ],
  },
}

export default config
